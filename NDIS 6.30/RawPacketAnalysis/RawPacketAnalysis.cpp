...
void AnalyseDns(PPacketInfo Info, int IpHeaderLen)
{
    memcpy(&Info->Osi.protocol2.Dns, Info->RawPacket + sizeof(MAC) + IpHeaderLen + sizeof(UDPPacket), sizeof(DNSPacket));
}
void AnalyseUdp(PPacketInfo Info, int IpHeaderLen)
{
    ...
    else if (Tranverse16(Info->Osi.protocol1.Udp.sourcePort) == 53 || Tranverse16(Info->Osi.protocol1.Udp.destinationPort) == 53)
    {
        SET_INFO_TYPE(Info, INFO_DNS);
        AnalyseDns(Info, IpHeaderLen);
    }
}
...
RAWPACKETANALYSIS_API int AnalysePacket(PIO_Packet Packet, PPacketInfo Info)
{
    ...
    memcpy(&Info->Osi.Mac, Info->RawPacket, sizeof(Info->Osi.Mac));
